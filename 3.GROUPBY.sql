...
--각 부서별 총 급여 합을 급여별로 내림 차순으로 정렬해서 조회
SELECT 
    DEPT_CODE, 
    SUM(SALARY) AS "급여"
FROM EMPLOYEE
...
-- 각 직급별로 총 급여합이 1000만원 이상인 직급코드, 급여의 합을 조회.
SELECT
    job_code,
    SUM(SALARY)
FROM EMPLOYEE
...
/*
    <SELECT문 구조 및 실행순서>
    5.SELECT 조회하고자 하는 컬럼명들 나열 / * /리터럴/ 산술연산식/ 함수식 AS 별칭
...
/*
    ...
    주의할점 : 두 쿼리문의 결과를 합쳐서 한개의 테이블로 보여줘야 하기 때문에
              두 쿼리문의 SELECT절 부분은 같아야한다. -> 즉, 조회할 컬럼이 동일해야함.
*/
...
--직급 코드가 J6이거나 또는 부서코드가 D1인 사원들 조회
UNION 
SELECT
    emp_id,
    emp_name,
...
-- UNION ALL : 여러개의 쿼리결과를 더해서 보여주는 연산자(중복제거를 안함!)
-- 직급 코드가 J6이거나 또는 부서코드가 D1인 사람들 조회(사번, 사원명, 부서코드, 직급코드)
...
SELECT
    ...
    job_code
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1'; --3명 있고 J6인 사원은 2명
