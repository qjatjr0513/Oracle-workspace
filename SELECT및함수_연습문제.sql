...
SELECT 
    emp_name,
    TO_CHAR(salary, 'L9,999,999') AS "급여"
FROM EMPLOYEE;
...
SELECT 
   ...
   TO_NUMBER(SUBSTR(emp_no, 1, 6)+ SUBSTR(emp_no, 8, 14)) AS "주민번호(앞,뒤) 합"
FROM EMPLOYEE
WHERE EMP_ID = '201';

--24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT 
    SUM((SALARY+SALARY*NVL(BONUS, 0))*12) AS "총 연봉"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
SELECT 
    ...
    COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE),'2002', 2)) AS "2002년",
    COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE),'2003', 3)) AS "2003년",
    COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE),'2004', 4)) AS "2004년"
FROM EMPLOYEE;

SELECT 
    COUNT(*) AS "전체 직원 수",
    COUNT(CASE WHEN EXTRACT(YEAR FROM HIRE_DATE) = 2001 THEN 123 END) AS "2001년",
    COUNT(CASE WHEN EXTRACT(YEAR FROM HIRE_DATE) = 2002 THEN 123 END) AS "2002년",
    COUNT(CASE WHEN EXTRACT(YEAR FROM HIRE_DATE) = 2003 THEN 123 END) AS "2003년",
    COUNT(CASE WHEN EXTRACT(YEAR FROM HIRE_DATE) = 2004 THEN 123 END) AS "2004년"
FROM EMPLOYEE;